...
from config import lambda_T, lambda_inlet, lambda_bd, lambda_outlet, batch_size, net2_path
...
def train(net2, optimizer, scheduler, epochs, encrypted_points, inlet_points, outlet_points, bd1_points, bd2_points, train_path, cfd_path, mean_vals, his_freq, batch_size):
    ...
    for epoch in range(epochs):
        ...
        for i in range(num_batches):
            ...
            T_loss = pde_loss_poisson(net2, batch_points)
            loss_pde = T_loss

            inlet_loss, outlet_loss = inlet_outlet_loss_poisson(net2, inlet_points, outlet_points)
            bd1_loss, bd2_loss = boundary_loss_poisson(net2, bd1_points, bd2_points)
