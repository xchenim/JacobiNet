...
def plot_velocity_pressure(net3, points_list, min_vals, max_vals, x0=0.0, tol=None):
    ...
    fig, ax = plt.subplots(figsize=(plt_x*0.9, plt_y*0.9))
    sc = ax.scatter(Y, Z, c=mag * U_non, cmap='coolwarm', s=1, vmin=0, vmax=v_max)
    plt.colorbar(sc, ax=ax, orientation='vertical', label='Velocity Magnitude')
...
def print_inlet_outlet_pressure(net3, inlet_points, outlet_points):
    net3.eval()
    with torch.no_grad():
        inlet_pressure = net3(inlet_points)[:, 2].cpu().numpy().flatten() * P_non
        outlet_pressure = net3(outlet_points)[:, 2].cpu().numpy().flatten() * P_non
...
if __name__ == "__main__":
    ...
    metrics = compute_l2_error_3d_full(net3, x_cfd, y_cfd, z_cfd, u_cfd, v_cfd, w_cfd, p_cfd, mean_vals, verbose=True)
