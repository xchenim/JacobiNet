...
from config import L, U_non, P_non, data_path, cfd_path, post_path, net2_u_path, v_max, vmax_error, p_max, pmax_error, plt_x, plt_y
from data_loader import load_cfd_data, load_train_data, read_groundtruth_csv, save_history_data
...
def plot_velocity_magnitude(net2, points_list, min_vals, max_vals, x0=0.0, tol=None, vec_scale=2000, s=1):
    net2.eval()
    with torch.no_grad():
...
def plot_velocity_pressure(net2, points_list, min_vals, max_vals, x0=0.0, tol=None):
    net2.eval()
    with torch.no_grad():
    ...
    fig, ax = plt.subplots(figsize=(plt_x, plt_y))
    sc = ax.scatter(Y, Z, c=mag * U_non, cmap='coolwarm', s=2, vmin=0, vmax=v_max)
    plt.colorbar(sc, ax=ax, orientation='vertical', label='Velocity Magnitude')
    ax.set_title('Velocity Magnitude on YZ @ x≈0')
    ax.set_xlabel('Y'); ax.set_ylabel('Z'); ax.set_aspect('equal', 'box'); plt.tight_layout(); plt.show()


    fig, ax = plt.subplots(figsize=(plt_x, plt_y))
    sc = ax.scatter(Y, Z, c=np.abs(U) * U_non, cmap='coolwarm', s=2, vmin=0, vmax=v_max)
    plt.colorbar(sc, ax=ax, orientation='vertical', label='|u|')
    ax.set_title('u on YZ @ x≈0')
    ax.set_xlabel('Y'); ax.set_ylabel('Z'); ax.set_aspect('equal', 'box'); plt.tight_layout(); plt.show()


    fig, ax = plt.subplots(figsize=(plt_x, plt_y))
    sc = ax.scatter(Y, Z, c=np.abs(V) * U_non, cmap='coolwarm', s=2, vmin=0, vmax=v_max)
    plt.colorbar(sc, ax=ax, orientation='vertical', label='|v|')
    ax.set_title('v on YZ @ x≈0')
    ax.set_xlabel('Y'); ax.set_ylabel('Z'); ax.set_aspect('equal', 'box'); plt.tight_layout(); plt.show()


    fig, ax = plt.subplots(figsize=(plt_x, plt_y))
    sc = ax.scatter(Y, Z, c=np.abs(W) * U_non, cmap='coolwarm', s=2, vmin=0, vmax=v_max)
    plt.colorbar(sc, ax=ax, orientation='vertical', label='|w|')
    ax.set_title('w on YZ @ x≈0')
    ax.set_xlabel('Y'); ax.set_ylabel('Z'); ax.set_aspect('equal', 'box'); plt.tight_layout(); plt.show()


    fig, ax = plt.subplots(figsize=(plt_x, plt_y))
    sc = ax.scatter(Y, Z, c=np.abs(P) * P_non, cmap='coolwarm', s=2, vmin=0, vmax=p_max)
    plt.colorbar(sc, ax=ax, orientation='vertical', label='Pressure')
    ax.set_title('Pressure on YZ @ x≈0')
    ax.set_xlabel('Y'); ax.set_ylabel('Z'); ax.set_aspect('equal', 'box'); plt.tight_layout(); plt.show()

def plot_error(
    x_cfd, y_cfd, z_cfd, u_cfd, v_cfd, w_cfd, p_cfd,
    net2, points_list, min_vals, max_vals,
    x0=0.0, tol=None, method='linear'
):
    net2.eval()
    with torch.no_grad():

        all_points = torch.cat(points_list, dim=0).to(next(net2.parameters()).device)
        outputs = net2(all_points)
...
def plot_error_Z(
    ...
    fig, axs = plt.subplots(1, 5, figsize=(plt_x*4, plt_y*2))
    axs[0].scatter(xq, yq, c=np.abs(err_vel), cmap='coolwarm', s=1, vmin=0, vmax=vmax_error)
    axs[0].set_title('Error |U|'); axs[0].set_aspect('equal','box'); axs[0].axis('off')

    axs[1].scatter(xq, yq, c=np.abs(err_u), cmap='coolwarm', s=1, vmin=0, vmax=vmax_error)
    axs[1].set_title('Error u'); axs[1].set_aspect('equal','box'); axs[1].axis('off')

    axs[2].scatter(xq, yq, c=np.abs(err_v), cmap='coolwarm', s=1, vmin=0, vmax=vmax_error)
    axs[2].set_title('Error v'); axs[2].set_aspect('equal','box'); axs[2].axis('off')

    axs[3].scatter(xq, yq, c=np.abs(err_w), cmap='coolwarm', s=1, vmin=0, vmax=vmax_error)
    axs[3].set_title('Error w'); axs[3].set_aspect('equal','box'); axs[3].axis('off')

    axs[4].scatter(xq, yq, c=np.abs(err_p), cmap='coolwarm', s=1, vmin=0, vmax=pmax_error)
    axs[4].set_title('Error p'); axs[4].set_aspect('equal','box'); axs[4].axis('off')
...
def print_l2_error_3d_full(
    ...
    device = next(net2.parameters()).device
    net2.eval()
...
def print_inlet_outlet_pressure(net2, inlet_points, outlet_points):
    net2.eval()
    with torch.no_grad():
        inlet_pressure = net2(inlet_points)[:, 2].cpu().numpy().flatten() * P_non
        outlet_pressure = net2(outlet_points)[:, 2].cpu().numpy().flatten() * P_non

        mean_inlet_pressure = np.mean(inlet_pressure)
        mean_outlet_pressure = np.mean(outlet_pressure)

        print(f"Pressure Drop: {mean_inlet_pressure - mean_outlet_pressure:.2f} Pa")
...
if __name__ == "__main__":
    ...
    state_dict = torch.load(net2_u_path, map_location=device, weights_only=True)
    net2.load_state_dict(state_dict)


    plot_error_Z(x_cfd, y_cfd, z_cfd, u_cfd, v_cfd, w_cfd, p_cfd, net2, points_list, min_vals, max_vals)


    metrics = print_l2_error_3d_full(net2, x_cfd, y_cfd, z_cfd, u_cfd, v_cfd, w_cfd, p_cfd, mean_vals, verbose=True)
