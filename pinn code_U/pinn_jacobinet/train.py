...
from config import lambda_T, lambda_inlet, lambda_bd, lambda_outlet, net3_path, T_bd1, T_bd2
...
def train(net3, optimizer, scheduler, epochs, train_path, cfd_path, his_freq, batch_size, xy_points, encrypted_xy, inlet_xy, outlet_xy, bd1_xy, bd2_xy, jacobinet_path, mean_vals_xy):
    ...
    for epoch in range(epochs):
        ...
        for i in range(num_batches):
            ...
            T_loss = pde_loss_laplace(net3, batch_points_xy, Q_func=gaussian_source)
            loss_pde = T_loss

            inlet_loss, outlet_loss = inlet_outlet_loss_laplace(net3, inlet_xy, outlet_xy)
            bd1_loss, bd2_loss = boundary_loss_laplace(net3, bd1_xy, bd2_xy, T_bd1, T_bd2)
            ...
            total_loss = lam_T * loss_pde + lam_inlet * inlet_loss + lam_outlet * outlet_loss
